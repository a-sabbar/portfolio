...
export default function Body({ setScaleValue } = props) {
    ...
    return (
        <div className="flex h-screen items-center justify-center p-24 w-screen bg-black  overflow-hidden cursor-default relative">
            <div className="flex flex-col items-center justify-center gap-[20px] z-[1]">
                <a href={"/front-end_Achraf-Sabbar.pdf"} download>
                    <Image src="/myImage.jpg" alt="" width={200} height={200} className="rounded-full w-48 h-48  object-cover outline outline-white outline-[2px]" />
                </a>
                <>
                    <div className="text-white text-[35px] font-[400] poppins tracking-wide"
                        onMouseEnter={() => setScaleValue(3)}
                        onMouseLeave={() => setScaleValue(1)}>
                        Achraf Sabbar
                    </div>
                    <div className="text-white text-[55px] font-[500] lato -mt-[25px] "
                        onMouseEnter={() => setScaleValue(3)}
                        onMouseLeave={() => setScaleValue(1)}>
                        Front-End Developer
                    </div>
                </>
                <motion.p className="text-white text-[25px] font-[400] lusitana mt-[20px] opacity-0"
            ...
            <span className="absolute bottom-[150px] left-1/2 z-[1]" ref={scroll}>
